...
#######################################################################################################################
    ...
    def _gen_embed(self):
        ...
        e = discord.Embed()
        e.colour = 7506394
        e.set_footer(text='MrGameX/MusicBot ({})'.format(BOTVERSION), icon_url='https://i.imgur.com/gFHBoZA.png')
        e.set_author(name=self.user.name, url='https://github.com/MrGameX/MusicBot', icon_url=self.user.avatar_url)
        return e
    ...
    async def on_message(self, message):
        if message.content == "!ping":
            await self.safe_send_message(message.channel, 'pong')
        await self.wait_until_ready()

        if message.content == "!delete":
            await self.safe_send_message(message.channel, "test")
            amount = 3
            channel = message.channel
            messages = []
            async for message in client.logs_from(channel, limit=int(amount) + 1):
                messages.append(message)
            await self.safe_delete_message(messages, quiet=True)
            await self.safe_send_message(message.channel, "Messages deleted")
